 ...
 %       end
    ...
    for episode = 1:numEpisodes
        ...
        if ~isempty(FoundSpikes.pks{episode})%this episode spiked
            ...
            %take average over 20 points = 2ms
            preSpikeRange = (pkStart-2*window):pkStart;
            R.threshold{episode} = mean(V(preSpikeRange, episode));
            R.rheobase{episode} = mean(I(preSpikeRange, episode));
